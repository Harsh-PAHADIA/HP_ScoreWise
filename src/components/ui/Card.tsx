...
> = ({ children, className, as: Tag = 'h3' }) => {
  return (
    <Tag className={cn('text-2xl font-semibold leading-none tracking-tight text-black', className)}>
      {children}
    </Tag>
  );
};
...
export const CardFooter: React.FC<CardProps> = ({ children, className }) => {
  return (
    <div className={cn('flex items-center pt-4 mt-4 border-t border-neutral-100', className)}>
      {children}
    </div>
  );
};